...
def extraer_numero(text):
    ...
    return float(numbers[0]) if numbers else pd.np.nan
	
	
term_list = ['months']
data['term_info']=data['term'].map(lambda x: substrings_in_string(x, term_list))
data['term_num'] = data['term'].apply(extraer_numero)